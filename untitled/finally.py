...
import os, sys
img= Image.open(r'C:\Users\hggag\Desktop\gurke.jpg')
...
loecher_fuellen= int(input("Wenn im Closing Löcher innerhalb des Objekts vorhanden sind, die gefüllt werden sollen, gib 0 ein, sonst 1:"))

if loecher_fuellen==0:
    unique, counts = np.unique(neues_bild, return_counts=True)
    f = dict(zip(unique, counts))
    d = {'weiß': f[255], 'schwarz': f[0]}
    print(d)
    cv.imshow("Gefülltes Bild", neues_bild)
